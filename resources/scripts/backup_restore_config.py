...
parser.add_argument(
    ...
    type=str,
    default="backup",
    help="Either backup or restore the workspace configuration.",
...
if args.mode == "restore":
    if (
        CONFIG_BACKUP_ENABLED is None
        or CONFIG_BACKUP_ENABLED.lower() == "false"
        or CONFIG_BACKUP_ENABLED.lower() == "off"
    ):
        log.info(
            "Configuration Backup is not activated. Restore process will not be started."
        )
        sys.exit()
    ...
    if (
        not os.path.exists(CONFIG_BACKUP_FOLDER)
        or len(os.listdir(CONFIG_BACKUP_FOLDER)) == 0
    ):
    ...
    # set verbose? -v
    rsync_restore = (
        "rsync -a -r -t -z -E -X -A " + CONFIG_BACKUP_FOLDER + " " + USER_HOME
    )
    ...
    # set verbose? -v
    rsync_backup = (
        "rsync -a -r -t -z -E -X -A --delete-excluded --max-size=100m \
                        "
        + backup_selection
        + " \
    ...
    command = (
        ...
        + " '"
        + script_file_path
        + "' backup> /proc/1/fd/1 2>/proc/1/fd/2"
    )

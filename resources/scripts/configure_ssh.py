...
if not os.path.isfile(HOME + "/.ssh/" + SSH_KEY_NAME):
    ...
    call(
        'ssh-keygen -f ~/.ssh/{} -t ed25519 -q -N "" > /dev/null'.format(SSH_KEY_NAME),
        shell=True,
    )

# Copy public key to resources, otherwise nginx is not able to serve it
call(
    "/bin/cp -rf " + HOME + "/.ssh/id_ed25519.pub /resources/public-key.pub", shell=True
)
...
call(
    'grep -qxF "$(cat {home}/.ssh/{key_name}.pub)" {home}/.ssh/authorized_keys || cat {home}/.ssh/{key_name}.pub >> {home}/.ssh/authorized_keys'.format(
        home=HOME, key_name=SSH_KEY_NAME
    ),
    shell=True,
)

...
call(
    ...
    shell=True,
)  # create a link in categories menu to your Jupyter notebook server
...
shortcut_metadata = (
    "[Desktop Entry]\nVersion=1.0\nType=Link\nName=Jupyter Lab\nComment=\nCategories=Development;\nIcon="
    + ENV_RESOURCES_PATH
    + "/icons/jupyterlab-icon.png\nURL="
    + url
)

call(
    'printf "' + shortcut_metadata + '" > /usr/share/applications/jupyterlab.desktop',
    shell=True,
)  # create a link in categories menu to your Jupyter Lab server
call(
    "chmod +x /usr/share/applications/jupyterlab.desktop", shell=True
)  # Make executable
...
if not os.path.exists(HOME + "/filebrowser.db"):
    ...
    call(
        "filebrowser config init --database=" + HOME + "/filebrowser.db > /dev/null",
        shell=True,
    )
    ...
    call(
        ...
        + filebrowser_pwd
        + " --perm.admin=true --database="
        + HOME
        + "/filebrowser.db > /dev/null",
        shell=True,
    )

    # Configure filebrowser
    configure_filebrowser = (
        'filebrowser config set --root="/" --auth.method=proxy --auth.header=X-Token-Header '
